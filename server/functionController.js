...
profile = (req, res) => {
	...
	const profileIndex = db.profiles.data.findIndex(
		prof => prof.p_id === parseInt(usersId)
	);
...
fetchProfile = (req, res) => {
	...
	if (req.query.email) {
		...
		const profileIndex = db.profiles.data.findIndex(
			profile =>
				profile.p_id === parseInt(db.users.data[userIndex].u_id)
		);
	...
	if (req.query.id) {
		const userIndex = db.users.data.findIndex(
			user => user.u_id === parseInt(req.query.id)
		);
		const profileIndex = db.profiles.data.findIndex(
			profile =>
				profile.p_id === parseInt(db.users.data[userIndex].u_id)
...
function viewPost(req, res) {
	...
	if (req.query.comments === "") {
		...
		const postIndex = db.posts.data.findIndex(
